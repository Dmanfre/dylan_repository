...
Exporting data for graphs

```{r}
#D1 players from Maryland

view(division_one_md)
division_one_md <- write_csv(division_one_md, "data/division_one_md.csv")

#us state totals with d1 players
D1_usa_2022_with_state_counts <- write_csv(D1_usa_2022_with_state_counts, "data/D1_usa_2022_with_state_counts.csv")


```


Question: Which High School has the most players? Is there a pipeline between any school?
```{r}
...
Cleaning the data based on what is in the wbb_1129 rmd.file
...
Data exploration
I want to move the division column in 22-23 to the 23-24 dataframe.
```{r}
#putting the divisions into 23-24
...
D1 state counts:
How many D1 players are from each state
```{r}
...
D1 from Maryland counts
How many D1 players are from Maryland?
114 Division 1 players are from Maryland.

Reason could be because of graduation
```{r}
